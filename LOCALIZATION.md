...
Both `mifos-authenticator-passcode` and `cmp-sample-shared` ship Compose Multiplatform Resource bundles for the same 39 locales as the [`openMF/kmp-project-template`](https://github.com/openMF/kmp-project-template) reference app. Default text is English (`values/`).
...
| `values-it` | Italian |
| `values-iw` | Hebrew (legacy code; modern is `he`) |
| `values-ja` | Japanese |
| `values-ko` | Korean |
| `values-lv` | Latvian |
| `values-ml` | Malayalam |
...
>
> Specifically flagged for native review: **Thai (`th`)**, **Persian (`fa`)**, **Malayalam (`ml`)** — auditor declined to certify these. All other locales should be reviewed by a fluent speaker before shipping to that market, particularly the smaller European languages (`af`, `be`, `et`, `lv`, `ca`).
>
